...
func drawImagesOnBackgroundInParallel(numberOfRows int, shape ImageShape, maxWidth uint, maxHeight uint, maxNumberOfColumns int, imagesMatrix [][]image.Image, desiredWidth int) *MyImage {
	...
	for r := range imagesMatrix {
		for c := range imagesMatrix[r] {
			drawSingleImageOnBackground(imagesMatrix[r][c], imagesMatrix, padding, shape, desiredWidth, &output)
		}
	}
...
func saveImage(img image.Image, filename string) {
	...
	if err != nil {
		log.Fatal("Error creating the output file")
	}
	...
	if err != nil {
		log.Fatal("Error encoding and saving the image")
	}
			...
			for {
				...
				case img := <-imagesChannel:
					...
					if len(images) == imagesCount {
						...
						saveImage(output, "output_image.jpg")

						exec_duration := time.Since(exec_start)
						//print
						// fmt.Println("\n\n")
						log.Print("Total execution time in Go " + exec_duration.String())
						return
					}
				case <-errChannel:
